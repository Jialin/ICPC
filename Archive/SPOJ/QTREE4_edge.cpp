...
int n;
gs::EdgeWeightedGraph<int> tree;
trees::DivideConquerEdgeDecomposor<int> de;
bool white[MAXN4];
int bestPath[MAXN4];
priority_queue<pair<int, int>> heap[MAXN4];
...
int main() {
  ...
  FOR(i, 1, n) {
    ...
    tree.addUndirected(x - 1, y - 1, w);
  }
  de.calc(tree);
  fill(bestPath, bestPath + de.compCnt, -INF);
  FOR(i, 0, n) update(i);
  int q; scanf("%d", &q);
  FOR(_, 0, q) {
    scanf("%s", op);
    if (op[0] == 'A') {
      int& res = bestPath[de.compCnt - 1];
      if (res == -INF) {
