...
struct BaseGroupingIntervalsContainer {
  ...
  inline IntervalIter emplace(Key lower, Key upper, GroupID id) {
    ...
    if (frontLower < frontUpper) {
      _afterInsert(_intervals.emplace_hint(jt, move(frontLower), move(frontUpper), move(frontID)));
    }
    auto res = _intervals.emplace_hint(jt, move(interval));
    _afterInsert(res);
    if (backLower < backUpper) {
      _afterInsert(_intervals.emplace_hint(jt, move(backLower), move(backUpper), move(backID)));
    }
    return res;
  }
