...
TEST(MinSparseTableArrayTest, basic) {
  MinSparseTableArray<int, 3, 5> st;
  st.init({3, 2, 1, 4, 5});
  ...
  EXPECT_EQ(1, st.calc(1, 3));
  EXPECT_EQ(1, st.calc(2, 4));
  EXPECT_EQ(4, st.calc(3, 5));

  EXPECT_EQ(1, st.calc(0, 3));
  EXPECT_EQ(1, st.calc(1, 4));
  EXPECT_EQ(1, st.calc(2, 5));

  EXPECT_EQ(1, st.calc(0, 4));
  EXPECT_EQ(1, st.calc(1, 5));

  EXPECT_EQ(1, st.calc(0, 5));
}
