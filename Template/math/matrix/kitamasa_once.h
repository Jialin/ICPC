...
struct KitamasaOnce {
  ...
  inline V calc(const vector<V>& coef, const vector<V>& xs, K k, V mod) {
  ...
  template<typename K = int>
  inline void _calc(const vector<V>& coef, K k) {
    if (k < _n) {
      _res.assign(_n, 0);
    ...
    if (k & 1) {
      ...
      for (int i = _n - 1; i > 0; --i) {
        _res[i] = addMod<V>(
            _res[i - 1], mulMod<V, V_SQR>(lastRes, coef[i], _mod), _mod);
      }
      _res[0] = mulMod<V, V_SQR>(lastRes, coef[0], _mod);
    } else {
      _calc(coef, k >> 1);
      for (int i = 0; i < _n21; ++i) {
        _tmpRes[i].reset(0);
