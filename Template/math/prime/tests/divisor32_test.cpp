...
TEST(DivisorIterator32Test, small) {
  PrimeGenerator generator(4);
  vector<int> ds;
  ...
  divisorIterator32(4, generator.primes, processor);
  ...
  ds.clear();
  divisorIterator32(5, generator.primes, processor);
  sort(ds.begin(), ds.end());
  ASSERT_EQ(ds.size(), 2);
  EXPECT_EQ(ds[0], 1);
  EXPECT_EQ(ds[1], 5);
  ...
  ds.clear();
  divisorIterator32(11, generator.primes, processor);
  sort(ds.begin(), ds.end());
  ASSERT_EQ(ds.size(), 2);
  EXPECT_EQ(ds[0], 1);
  EXPECT_EQ(ds[1], 11);
...
TEST(DivisorIterator64Test, large) {
  PrimeGenerator generator(1000);
  vector<int> ds;
  auto processor = [&ds](int d) {
    ds.push_back(d);
    return true;
  ...
  divisorIterator32(123456, generator.primes, processor);
  sort(ds.begin(), ds.end());
  ...
  ds.clear();
  divisorIterator32(987654, generator.primes, processor);
  sort(ds.begin(), ds.end());
