...
template <typename T = LL> class Fraction {
public:
  inline Fraction() {}
  ...
  inline void initZero() { init(0, 1, false); }

  inline void initOne() { init(1, 1, false); }

  inline void initMediant(const Fraction &l, const Fraction &r,
                          bool fixCoprime = true) {
    num_ = l.num_ + r.num_;
    den_ = l.den_ + r.den_;
    if (fixCoprime) {
      fixCoprime_();
  ...
  inline void updateInverse() { swap(num_, den_); }
  ...
  inline bool operator>=(const Fraction &o) const { return !(*this < o); }
  ...
  inline Fraction operator+(const Fraction& o) const {
    ...
    return Fraction(num_ * (newDen / den_) + o.num_ * (newDen / o.den_),
  newDen);
  }
...
template <typename T> string toDisplay(const math::Fraction<T> &f) {
  ostringstream ss;
  ss << f.num() << "/" << f.den();
  return ss.str();
}
